...
A. Guzman-Ballen\\
       ...
       \email{\large\texttt{aguzman4@illinois.edu}}
% 2nd. author
\alignauthor
E. Montagner\\
       \affaddr{University of Illinois at \\ Urbana - Champaign}\\
       \email{\large\texttt{montgnr2@illinois.edu}}
% 3rd. author
\alignauthor
J. V. Ruiz Cepeda\\
       \affaddr{University of Illinois at \\ Urbana - Champaign}\\
       \email{\large\texttt{ruizcep2@illinois.edu}}
...
Questions about the origin of the universe have been part of humankind since the beginning of times. In this context, the classification of galaxies based on shape and size is important in order to better understand the distribution of galaxies within the universe and to provide answers to questions regarding their origin. The power of telescopes is increasing rapidly over time and, every day, they capture thousands of images of galaxies, so the amount of data available is growing exponentially. Images of galaxies have up until now been classified manually, so the need for automatic computational methods to classify them is of great importance. \\

Our goal is to use the dataset provided by \emph{Galaxy Zoo} to design a procedure that performs the classification automatically and reproduces the results of the manual classification.

\section{Classification approach}
\subsection{Data}
\subsection{Techniques}
 As part of the preprocessing, we used \emph{Otsu’s method} to obtain a binary image, which we then used to extract only the relevant galaxy in the middle of the picture. We also downsampled the input images to a size of 208x208 pixels.
\subsection{Results}
\subsection{Conclusions}
\section{Individual contribution}
Our team has held weekly meetings to make sure our project is making progress. When it comes to division of labor, it was decided that it would be determined based on the strengths and interests of each of the members. 

\begin{itemize}
       \item Andres, with some digital signal processing background, tried looking into the potential behind 2D Fourier Transforms to see if similar galaxies had similar Fourier Transforms. Ettienne and Andres, through researching different papers on the subject matter, managed to find a way to extract the center galaxies from the image in the process. This would be very beneficial for the actual algorithm that’s going to be classifying the galaxies because it simplifies the input data. 
       \item Jose worked on implementing and using SVMs and SIFT descriptors to classify the given galaxies. 
       \item Ettienne is researching other potential methods for feature extraction and for galaxy classification (at the moment, the use of eigenfaces and Neural Networks seem promising). We are all collectively still reading through papers, using MATLAB, attempting to incrementally get this project working, and using GitHub has made this very easy to do.
\end{itemize}
...
Results: Clearly describe your experimental protocols. If you are using training and test data, report the numbers of training and test images. Be sure to include example output figures. Quantitative evaluation is always a big plus (if applicable). If you are working with videos, put example output on YouTube or some other external repository and include links in your report.
